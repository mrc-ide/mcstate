...
test_that("pmcmc nested multivariate gaussian", {
  ...
  testthat::try_again(5, {
    ...
    i <- seq(1, 1000, by = 20)
    expect_s3_class(res, "mcstate_pmcmc")
    sw_ks.test = function(x, y) suppressWarnings(ks.test(x, y))
    expect_gt(sw_ks.test(res$pars[1, "a", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[2, "a", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[3, "a", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[1, "b", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[2, "b", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[3, "b", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[1, "c", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[2, "c", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[3, "c", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[1, "d", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[2, "d", i], "pnorm")$p.value, 0.05)
    expect_gt(sw_ks.test(res$pars[3, "d", i], "pnorm")$p.value, 0.05)
