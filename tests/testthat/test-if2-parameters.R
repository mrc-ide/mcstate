...
test_that("if2_parameter prior works", {
  expect_error(
    p <- if2_parameter("a", -0.1, min = -1, max = 1,
                       prior = function(x) log(x)),
...
test_that("can construct and walk a set of parameters", {
  ...
  expect_s3_class(pars, "smc2_parameters")
  expect_equal(pars$names(), c("beta", "gamma", "time"))
  expect_equal(pars$initial(), c(0.15, 0.05))
  ...
  expect_true(all(lapply(walk_mat[3, ], is.integer)))
