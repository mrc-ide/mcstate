...
```{r}
# Set up model
incidence <- t(read.table("sir_incidence_data.txt", sep=",", header = F))
incidence <- data.frame(cases = incidence[, 1], day = seq_len(100), row.names = NULL)
```

This can then be processed with `mcstate::particle_filter_data()` as follows, taking four update steps between every observation (generally decreasing $dt$ will improve resolution, especially if rates of change are high, at the expense of extra computation time):
...
r_output(readLines(file.path(path, "vignettes/deaths.R")))
```
...
```{r}
index <- function(info) {
  list(run = c(info$index$S, info$index$D), 
       state = c(info$index$S, info$index$I, info$index$R, info$index$D))
}
