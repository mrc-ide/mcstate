...
A simple definition of a nested SIR model is extended from the [odin documentation](https://mrc-ide.github.io/odin/articles/discrete.html) as:
$$\begin{align*}
...
We will model this example in by extending the example  from the `vignette("sir_models")` vignette.

## Model data

As with the `vignette("sir_models")` vignette, the daily counts have been generated from a single run of the model with `seed = 3L`.
...
Note that in contrast to regular SIR models, data for nested models should be in a long format and should include a factor column for populations. This is passed to `mcstate::particle_filter_data()`, which includes an argument for specifying which column includes the population identifier. 
...
The biggest difference between the two cases lies in the `mcstate::pmcmc_parameters_nested()` and associated `mcstate::pmcmc_varied_parameter()` function. We will separately look at the new function and then the object but first in the usual case where a parameter is *fixed* and does not vary between populations, then the usual `mcstate::pmcmc_parameter()` function can be used:

```{r}
beta <- mcstate::pmcmc_parameter("beta", 0.2, min = 0)
```

For the gamma parameter, which varies between populations, we instead use `mcstate::pmcmc_parameter()`:
...
In this example the two populations have different initial values, but the same prior and range of values the parameter can take. Note that this function also takes the new argument `populations` to specify the names of the populations.

Now we can create a `mcstate::pmcmc_parameters_nested()` object, as with the un-nested case we require a proposal matrix for parameters. However for this object we have one matrix for fixed parameters and one for varied parameters. Currently fixed/varied parameters are dependent on other fixed/varied parameters only and are independent of varied/fixed parameters.

```{r}
proposal_fixed <- matrix(0.01)
row.names(proposal_fixed) <- colnames(proposal_fixed) <- "beta"
proposal_varied <- matrix(0.01)
...
Note that this final `populations` parameter is only strictly required if only fixed parameters are included in the object. Now we are ready to run the pMCMC.
...
We can now continue visualisation and further analysis as in the un-nested case, just be careful to include the additional dimension of population.
...
```{r}
plot_particle_filter <- function(history, true_history, times, population, obs_end = NULL) {
