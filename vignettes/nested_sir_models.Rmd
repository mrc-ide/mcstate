...
gamma <- mcstate::pmcmc_varied_parameter(
  ...
  prior = list(
    ...
    function(p) dgamma(p, shape = 1, scale = 0.25, log = TRUE)
  ),
  min = 0
)
```
...
As with the un-nested case we require a proposal matrix for parameters, however for this object we have one matrix for fixed parameters and one for varied parameters.
