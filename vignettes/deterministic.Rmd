...
First, run a stochastic fit, as in `vignette("sir_models")`. Here we use 500 particles to get a low-variance estimate of the likelood:
...
then again, with the deterministic filter. The only change is to use the object `mcstate::particle_nofilter` and omit the `n_particles` element to the constructor (there will only ever be one particle!) and the `n_threads` element which has no effect here.

```{r}
p2 <- mcstate::particle_nofilter$new(data, sir, compare, index = index)
